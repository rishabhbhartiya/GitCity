/**
 * CitySimulation.jsx — GitCity v9
 * Integrated: WeatherSystem (rain, thunder, tornado, spring/dry leaves)
 *             PedestrianSystem (realistic peds + 5 tree types)
 * Removed: old inline addTree + old inline pedestrian code
 */
...
// Weather modes cycle
const WEATHER_MODES = ["clear", "rain", "thunder", "tornado", "spring"];
...
export function CitySimulation({ cells, stats, theme, profile }) {
    ...
    const weatherRef = useRef(null); // WeatherSystem instance
    ...
    // Sync weather mode change to existing weather system
    useEffect(() => {
        if (weatherRef.current) weatherRef.current.setMode(weatherMode);
    }, [weatherMode]);
    ...
    useEffect(() => {
        ...
        sunDisc.position.set(300, 320, -200); scene.add(sunDisc); sunDisc.visible = false;

        const cloudGroup = new THREE.Group(); scene.add(cloudGroup); cloudGroup.visible = false;
        for (let i = 0; i < 10; i++) {
            const cg = new THREE.Group();
            for (let j = 0; j < 4; j++) {
                const cm = new THREE.Mesh(new THREE.SphereGeometry(7 + Math.random() * 5, 8, 8), new THREE.MeshLambertMaterial({ color: 0xffffff, transparent: true, opacity: 0.82 }));
                cm.position.set(j * 9 - 13, Math.random() * 4, Math.random() * 4); cg.add(cm);
            }
            cg.position.set((Math.random() - .5) * 300, 80 + Math.random() * 40, (Math.random() - .5) * 300);
            cloudGroup.add(cg);
        }
        ...
        cloudGroup.children.forEach(cg => {
            cg.position.set((Math.random() - .5) * cityW * 1.4, 80 + Math.random() * 40, (Math.random() - .5) * cityD * 1.4);
        });
        ...
        weather.setMode(weatherMode); // apply current state
        ...
        function drawRoad(x1, z1, x2, z2, w = ROAD) {
            ...
            rm.position.set((x1 + x2) / 2, 0.09, (z1 + z2) / 2); rm.rotation.y = angle; rm.receiveShadow = true; scene.add(rm);
            for (let i = 0, n = Math.floor(len / 10); i < n; i++) {
                const t = (i + 0.5) / n;
                const d = new THREE.Mesh(new THREE.BoxGeometry(0.35, 0.15, 3.5), markMat);
                d.position.set(x1 + dx * t, 0.15, z1 + dz * t); d.rotation.y = angle; scene.add(d);
            }
        }
        ...
        for (let r = 0; r <= rows; r++) {
            ...
            for (let c = 0; c < cols; c++) {
                const above = r > 0 && hasDistrict(r - 1, c);
                const below = r < rows && hasDistrict(r, c);
            ...
            for (let c = minC; c <= maxC; c++) {
                ...
                const t1 = treeTypes[treeTypeIdx % treeTypes.length]; treeTypeIdx++;
                const t2 = treeTypes[treeTypeIdx % treeTypes.length]; treeTypeIdx++;
                addTree(scene, THREE, tx, rz - ROAD / 2 - 1.5, t1, 0.85);
                addTree(scene, THREE, tx, rz + ROAD / 2 + 1.5, t2, 0.85);
        ...
        for (let c = 0; c <= cols; c++) {
            ...
            let minR = -1, maxR = -1;
            for (let r = 0; r < rows; r++) {
                const left = c > 0 && hasDistrict(r, c - 1);
                const right = c < cols && hasDistrict(r, c);
            ...
            for (let r = minR; r <= maxR; r++) {
                ...
                const t1 = treeTypes[treeTypeIdx % treeTypes.length]; treeTypeIdx++;
                const t2 = treeTypes[treeTypeIdx % treeTypes.length]; treeTypeIdx++;
                addTree(scene, THREE, rx - ROAD / 2 - 1.5, tz, t1, 0.85);
                addTree(scene, THREE, rx + ROAD / 2 + 1.5, tz, t2, 0.85);
            }
        }
        ...
        districts.forEach((dist, di) => {
            ...
            sw.position.set(bx0 + BW / 2, 0.14, bz0 + BD / 2); sw.receiveShadow = true; scene.add(sw);
        });
        ...
        if (peakBeam && peakBeam.wx !== undefined) {
            ...
            for (let ri = 0; ri < 12; ri++) {
                ...
                ray.position.set(midX, midY, midZ); ray.rotation.z = tiltAngle; ray.rotation.y = angle; scene.add(ray);
        ...
        function buildPlane(sc = 1) {
            ...
            [[1, -0.2, 6], [1, -0.2, -6]].forEach(([rx, ry, rz]) => {
                ...
                const wlet = new THREE.Mesh(new THREE.BoxGeometry(0.7 * sc, 1.4 * sc, 0.12 * sc), wM); wlet.position.set(rx * sc, 0.8 * sc, (rz > 0 ? 12.5 : -12.5) * sc); pg.add(wlet);
        ...
        function moveOnRoad(obj, dt, spd) {
            ...
            if (obj.t > 1 || obj.t < 0) {
                ...
                roadSegs.forEach((s, si) => { if (si === obj.segIdx) return; const d = Math.min(Math.sqrt((s.x1 - ex) ** 2 + (s.z1 - ez) ** 2), Math.sqrt((s.x2 - ex) ** 2 + (s.z2 - ez) ** 2)); if (d < bd2 && d < ROAD * 1.5) { bd2 = d; bi = si; } }); obj.segIdx = bi; if (Math.random() > 0.7) obj.dir *= -1;
            }
            const s2 = roadSegs[obj.segIdx]; const t2 = Math.max(0, Math.min(1, obj.t));
            const px = s2.x1 + s2.dx * s2.len * t2, pz = s2.z1 + s2.dz * s2.len * t2;
            obj.group.position.set(px - s2.dz * obj.dir * (obj.laneOff || 1.8), 0, pz + s2.dx * obj.dir * (obj.laneOff || 1.8));
        ...
        function animate() {
            ...
            scene.background.set(skyColor); scene.fog.color.set(skyColor);
            ...
            hemi.intensity = NM ? 0.25 : 0.6; hemi.color.set(NM ? 0x0a1428 : 0x88bbff);
            ...
            if (fwdKey || bkKey) { const sn = snapRoad(car.position.x, car.position.z, roadSegs); if (sn.dist < ROAD * 0.9) { car.position.x += (sn.x - car.position.x) * 0.04; car.position.z += (sn.z - car.position.z) * 0.04; } }
            ...
            if (m.orbit) {
                ...
                camP.lerp(new THREE.Vector3(orbitTgt.x + dist * Math.sin(m.ox) * Math.cos(m.oy), orbitTgt.y + Math.max(4, dist * Math.sin(m.oy)) + 2, orbitTgt.z + dist * Math.cos(m.ox) * Math.cos(m.oy)), 0.1);
                camL.lerp(orbitTgt.clone().add(new THREE.Vector3(0, 1, 0)), 0.1);
            } else {
                const followDist = 14, followH = 5.5;
                const snapSpeed = (fwdKey || bkKey) ? 0.18 : 0.08;
                camP.lerp(new THREE.Vector3(car.position.x - fwdX * followDist, car.position.y + followH, car.position.z - fwdZ * followDist), snapSpeed);
                ...
                try {
                    ...
                    if (hits.length > 0) {
                        ...
                        if (hitObj && hitObj !== prevNearest) {
                            prevNearest = hitObj;
                            const p3 = new THREE.Vector3(hitObj.wx, hitObj.bH / 2, hitObj.wz); p3.project(camera);
    ...
    return (
        <div style={{ position: "relative", width: "100%", height: "100%" }}>
            ...
            {welcome && (
                <div style={{ position: "absolute", inset: 0, zIndex: 50, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", overflow: "hidden", background: `radial-gradient(ellipse 80% 70% at 50% 40%, ${theme.surface}ee 0%, ${theme.bg}ff 70%)`, backdropFilter: "blur(12px)" }}>
                    ...
                    <div style={{ position: "relative", textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", gap: "1.2rem" }}>
                        ...
                        <div style={{ fontSize: "4rem", fontWeight: 900, color: theme.accent, letterSpacing: "-0.02em", lineHeight: 1, textShadow: `0 0 40px ${theme.glow}90,0 0 80px ${theme.glow}40,0 0 120px ${theme.glow}20`, animation: "titleReveal 0.8s cubic-bezier(0.2,0,0,1) 0.3s both" }}>GitCity</div>
                        {username && (<div style={{ fontSize: "1.1rem", color: theme.text, fontWeight: 600, letterSpacing: "0.05em", animation: "fadeInUp 0.6s ease 0.7s both" }}><span style={{ color: theme.muted, fontWeight: 400 }}>@</span>{username}</div>)}
                        ...
                        <div style={{ fontSize: "0.55rem", color: theme.muted, letterSpacing: "0.12em", animation: "fadeInUp 0.6s ease 1.4s both" }}>LOADING CITY…</div>
                    </div>
                    <style>{`
            ...
            {/* Bottom right buttons — Day/Night + Weather */}
            <div style={{ position: "absolute", bottom: "0.6rem", right: "0.6rem", display: "flex", gap: "0.4rem", zIndex: 10 }}>
                {/* Weather cycle button */}
                <button onClick={cycleWeather} style={{ background: `${theme.surface}cc`, border: `1px solid ${weatherMode !== "clear" ? theme.accent : theme.border}`, borderRadius: 6, padding: "0.32rem 0.7rem", cursor: "pointer", color: weatherMode !== "clear" ? theme.accent : theme.muted, fontSize: "0.58rem", fontFamily: "monospace", letterSpacing: "0.08em" }}>
                    {WEATHER_LABELS[weatherMode]}
                </button>
                {/* Day/Night button */}
